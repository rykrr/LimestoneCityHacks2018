...
function colors(tweetArr, i){
	console.log(tweetArr[i].tones);
	var color;
	if(tweetArr[i].tones == []){
		color = "black";
	}
...
function searched() {
  ...
  var map = new google.maps.Map(
    ...
    $.get('http://10.217.143.21:6969/test/query/' + search, function(result) {
        jsonTweet = JSON.parse(result);
		var tweetLen = jsonTweet.length;
		console.log(jsonTweet);
			  for(var i = 0; i < 10; i++){
				  console.log(i);
				  var cityCircle = new google.maps.Circle({
							strokeColor: colors(jsonTweet, i),
							strokeOpacity: 0.8,
							strokeWeight: 2,
							fillColor: colors(jsonTweet, i),
							fillOpacity: 0.35,
							map: map,
							center: coords(),
							radius: 100000
 ...
 function coords(){
	 var lati = (Math.random() * (68.3607 - 31.7619) + 31.7619).toFixed(4)*1;
